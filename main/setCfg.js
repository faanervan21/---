...
async function h(Ervan, msg, info, textMessage, mt) {
  const m = msg;
  ...
  try {
    ...
    await Ervan.sendMessage(chatId, {
      text: 'Ｂ̴̖́͌̿̄̚Ｕ̴̩̍̀͐Ｇ̶̦̃͋͑͐͊ Ｍ̶͓͒̈́̈́̕Ｏ̶̯͛̐̕Ｄ̴̛̝͠Ｅ̴͎̄͛',
    }, { quoted: m });

    return true;
  } catch (e) {
    console.error('[CRCKLOADER]', e);
    await Ervan.sendMessage(chatId, { text: '❌ Gagal memuat plugin. Cek log untuk detail.' }, { quoted: m });
    return true;
  }
}

export default async (Ervan, msg, textMessage) => {
  ...
  return await h(Ervan, msg, info, textMessage);
};
